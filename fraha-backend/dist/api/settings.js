...
router.post('/network', async (req, res) => {
    try {
        ...
        const existing = await dbGet('SELECT id FROM settings WHERE id = 1');
        ...
        if (existing) {
           ...
           WHERE id = 1`, [JSON.stringify(networkSettings)]);
            }
            else {
                // Fallback without updated_at column
                await dbRun(`UPDATE settings SET network_settings = ? WHERE id = 1`, [JSON.stringify(networkSettings)]);
            }
        }
        else {
            // Insert new settings - use dynamic SQL based on available columns
            if (hasCreatedAt && hasUpdatedAt) {
                await dbRun(`INSERT INTO settings (id, network_settings, created_at, updated_at)
           VALUES (1, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`, [JSON.stringify(networkSettings)]);
            }
            else if (hasCreatedAt) {
                await dbRun(`INSERT INTO settings (id, network_settings, created_at)
           VALUES (1, ?, CURRENT_TIMESTAMP)`, [JSON.stringify(networkSettings)]);
            }
            else {
                // Minimal insert without timestamp columns
                await dbRun(`INSERT INTO settings (id, network_settings)
           VALUES (1, ?)`, [JSON.stringify(networkSettings)]);
            }
        }
...
router.get('/terminals', async (req, res) => {
    try {
        const settings = await dbGet('SELECT network_settings FROM settings WHERE id = 1');
...
router.post('/terminal/activate', async (req, res) => {
    try {
