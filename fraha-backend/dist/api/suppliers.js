...
router.get('/products/:productId/suppliers', async (req, res) => {
    ...
    `, [productId]);
        const currentSupplierStats = supplierHistory.find((s) => s.supplier_id === product.supplier_id);
        res.json({
            product: {
                id: product.id,
                name: product.name,
            ...
            summary: {
                ...
                price_range: supplierHistory.length > 0
                    ? Math.max(...supplierHistory.map((s) => s.worst_price)) -
                        Math.min(...supplierHistory.map((s) => s.best_price))
                    : 0,
                best_supplier: supplierHistory.length > 0
                    ? supplierHistory.reduce((best, current) => current.average_cost_price < best.average_cost_price ? current : best)
                    : null,
                most_reliable_supplier: supplierHistory.length > 0
                    ? supplierHistory.reduce((best, current) => current.total_restocks > best.total_restocks ? current : best)
                    : null
            }
        });
    }
...
router.post('/', async (req, res) => {
    try {
        ...
        const result = await dbRun('INSERT INTO suppliers (name, created_at) VALUES (?, datetime("now"))', [name.trim()]);
        res.json({
            success: true,
...
router.post('/:supplierId/bulk-restock', async (req, res) => {
    try {
        const supplierId = parseInt(req.params.supplierId);
        ...
        for (const item of restockItems) {
            ...
            try {
                await dbRun('UPDATE products SET quantity = quantity + ?, cost_price = ?, last_restocked = datetime("now") WHERE id = ?', [quantity, costPrice, productId]);
