...
router.get('/check', async (req, res) => {
    try {
        ...
        const adminUser = await dbGet('SELECT * FROM users WHERE id = 1');
        if (!adminUser) {
            console.log('🔄 Creating default admin user...');
            const hash = await bcryptjs_1.default.hash('admin', saltRounds);
...
router.post('/login', async (req, res) => {
    try {
        ...
        const user = await dbGet('SELECT * FROM users WHERE username = ?', [validator_1.default.escape(username)]);
        if (!user) {
...
router.post('/user', async (req, res) => {
    try {
        ...
        if (id) {
            ...
            if (password) {
                userData.password = await bcryptjs_1.default.hash(password, saltRounds);
                await dbRun(`
                    UPDATE users SET 
