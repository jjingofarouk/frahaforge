...
// === RESTOCK ===
router.post('/product/restock', async (req, res) => {
    try {
        ...
        const product = await dbGet('SELECT * FROM products WHERE id = ?', [productIdNum]);
        if (!product)
            return res.status(404).json({ error: 'Not Found', message: 'Product not found' });
        const supplier = await dbGet('SELECT * FROM suppliers WHERE id = ?', [supplierIdNum]);
        if (!supplier)
        ...
        await dbRun(`UPDATE products SET quantity = ?, cost_price = ?, supplier = ?, supplier_id = ?, last_restocked = ? WHERE id = ?`, [newQuantity, String(costPrice), supplier.name, supplierIdNum, new Date().toISOString(), productIdNum]);
        await dbRun(`INSERT INTO restock_history (
...
router.get('/products/search', async (req, res) => {
    try {
        ...
        const docs = await dbAll(`SELECT 
         p.*,
         p.cost_price AS costPrice,
         p.category AS category
       FROM products p
       WHERE p.name LIKE ? OR p.barcode LIKE ? OR p.category LIKE ?`, [searchQuery, searchQuery, searchQuery]);
        res.json(docs);
    }
    catch (err) {
...
router.put('/product', async (req, res) => {
    try {
        ...
        if (!productId)
            return res.status(400).json({ error: 'Bad Request', message: 'Product ID required' });
        const existing = await dbGet('SELECT * FROM products WHERE id = ?', [parseInt(String(productId))]);
        if (!existing)
            return res.status(404).json({ error: 'Not Found', message: 'Product not found' });
        const updateFields = {};
        const fieldMappings = {
        ...
        if (req.body.prescriptionRequired !== undefined)
            updateFields.prescription_required = req.body.prescriptionRequired ? 1 : 0;
        if (req.body.isControlledSubstance !== undefined)
            updateFields.is_controlled_substance = req.body.isControlledSubstance ? 1 : 0;
        if (req.body.requiresRefrigeration !== undefined)
            updateFields.requires_refrigeration = req.body.requiresRefrigeration ? 1 : 0;
        ...
        if (quantityIncreased) {
            ...
            try {
                ...
                if (supplierId) {
                    const supplier = await dbGet('SELECT name FROM suppliers WHERE id = ?', [supplierId]);
                    if (supplier) {
...
router.post('/product', async (req, res) => {
    try {
        ...
        if (!productId) {
            ...
            await dbRun(`INSERT INTO products (id, name, barcode, price, cost_price, category, quantity, min_stock, stock, expiration_date, img, description)
...
router.get('/product/:productId', async (req, res) => {
    ...
    try {
        const product = await dbGet(`SELECT 
         p.*,
         p.cost_price AS costPrice,
         p.category AS category
       FROM products p 
...
router.post('/product/sku', async (req, res) => {
    try {
        const sku = validator_1.default.escape(req.body.skuCode);
        const doc = await dbGet(`SELECT *, cost_price AS costPrice FROM products WHERE barcode = ?`, [parseInt(sku)]);
        res.send(doc);
    }
    catch (err) {
        console.error(err);
...
// === DECREMENT INVENTORY ===
router.decrementInventory = async (products) => {
