...
router.get('/expenses/analysis', async (req: Request, res: Response) => {
  try {
    ...
    const categoryBreakdownWithPercentage = categoryBreakdown.map((category: any) => ({
      ...category,
    ...
    const analysisData = {
      ...
      monthlyTrend: monthlyTrend.map((month: any) => ({
        month: month.month,
        amount: month.amount,
        count: month.count
      })),
