...
interface FinancialMetrics {
    ...
    topPerformingProduct?: ProductPerformance;
    lowestMarginProduct?: ProductPerformance;
}
...
interface ProductMarginAnalysis {
    productId: number;
    productName: string;
    category: string;
    sellingPrice: number;
    ...
    profitAmount: number;
    salesCount: number;
    status: 'High Margin' | 'Medium Margin' | 'Low Margin' | 'No Cost Data';
}
...
async function initializeSampleData() {
    try {
        const expenseCount = await dbGet('SELECT COUNT(*) as count FROM accounting') as { count: number };
...
router.get('/analytics/product-margins', async (req: Request, res: Response) => {
    try {
        ...
        if (marginThreshold) {
            const threshold = parseFloat(marginThreshold as string);
        ...
        const products = await dbAll(query, params) as ProductMarginAnalysis[];
...
router.get('/inventory/valuation', async (req: Request, res: Response) => {
    try {
        ...
        const stockCounts = await dbGet(`
            ...
            FROM products
        `) as { totalItems: number; lowStockItems: number; outOfStockItems: number };
        ...
        const categoryBreakdown = await dbAll(`
            ...
            GROUP BY category
            ORDER BY value DESC
        `) as Array<{ category: string; value: number; itemCount: number }>;
...
router.get('/analytics/product-performance', async (req: Request, res: Response) => {
    try {
        ...
        const periodStart = startDate ? new Date(startDate as string).toISOString() : new Date(new Date().getFullYear(), new Date().getMonth(), 1).toISOString();
        const periodEnd = endDate ? new Date(endDate as string).toISOString() : new Date().toISOString();
        ...
        const productPerformance = await dbAll(`
            ...
            LIMIT ?
        `, [periodStart, periodEnd, parseInt(limit as string)]) as Array<{
            productId: number;
            productName: string;
            revenue: number;
            cogs: number;
        ...
        const performanceData: ProductPerformance[] = productPerformance.map(item => {
...
router.get('/metrics', async (req: Request, res: Response) => {
    try {
        ...
        const periodStart = startDate ? new Date(startDate as string).toISOString() : todayStart;
        const periodEnd = endDate ? new Date(endDate as string).toISOString() : todayEnd;
        ...
        `, [periodStart, periodEnd]) as Transaction[];
        ...
        `, [periodStart, periodEnd]) as Expense[];
        ...
        const productsMissingCost = products.filter(product => 
            !product.costPrice || product.costPrice === '0' || product.costPrice === ''
        ).length;

        // Format product performance data
        const topPerformingProduct: ProductPerformance | undefined = topProduct ? {
        ...
        const lowestMarginProductData: ProductPerformance | undefined = lowestMarginProduct ? {
...
router.get('/profit-trends', async (req: Request, res: Response) => {
    try {
        const { startDate, endDate, period } = req.query;
        
        // Calculate date range
        const today = new Date();
        const periodStart = startDate ? new Date(startDate as string) : new Date(today.getFullYear(), today.getMonth(), today.getDate());
        const periodEnd = endDate ? new Date(endDate as string) : new Date(today.getFullYear(), today.getMonth(), today.getDate(), 23, 59, 59);

        // Get all transactions from SQLite
        const transactions = await dbAll('SELECT * FROM transactions WHERE status = 1') as Transaction[];

        // Get all products from SQLite
        const products = await dbAll('SELECT * FROM products') as Product[];

        // Get all expenses from SQLite
        const expenses = await dbAll('SELECT * FROM accounting') as Expense[];

        // Generate trend data based on period
        const trendData = generateProfitTrendData(
            transactions, 
            expenses, 
            products, 
            periodStart, 
...
router.get('/alerts/missing-costs', async (req: Request, res: Response) => {
    try {
...
router.get('/expenses', async (req: Request, res: Response) => {
    try {
        ...
        const expenses = await dbAll(query, params) as Expense[];
        res.json(expenses);
    } catch (err) {
...
router.put('/expenses/:id', async (req: Request, res: Response) => {
    try {
        ...
        const updated = await dbGet('SELECT * FROM accounting WHERE id = ?', [id]) as Expense;
        res.json(updated);
    } catch (err) {
...
router.get('/expenses/categories', async (req: Request, res: Response) => {
    try {
        const categories = await dbAll('SELECT DISTINCT category FROM accounting') as { category: string }[];
...
function generateProfitTrendData(
